...
sub initialize {
	...
	my $ragna_server_control_port = $config{ragnarokserver_control_port};
	my $first_ragna_port = $config{ragnarokserver_first_port};
	$roServer = Poseidon::RagnaServerHolder->new($ragna_server_control_port, $ragna_ip, $number_of_clients, $first_ragna_port);
	...
	$connectServer = new Poseidon::ConnectServer($connect_port, $connect_ip, $roServer);
...
sub __start {
	...
	while (1) {
		...
		#	print "RoServer ".Dumper($roServer)."\n";
		#	print "QueryServer ".Dumper($queryServer)."\n";
		#}
		sleep SLEEP_TIME;
	}
}
