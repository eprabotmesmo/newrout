...
sub onClientNew {
	...
	print "[RagnarokServer]-> Ragnarok Online client connected on port ".$self->getPort().".\n";	
}
...
sub onClientExit {
	...
	print "[RagnarokServer]-> Ragnarok Online client disconnected on port ".$self->getPort().".\n";
}
...
#		$self->{client}->send($data);
		...
		if (uc(unpack("H2", substr($msg, 7, 1))) . uc(unpack("H2", substr($msg, 6, 1))) eq '0228') {
			...
			print "[PoseidonServer]-> Received response from Ragnarok Online client [port1: " . $self->getPort . "] [Time: " . time . "]\n";
		...
		print "[PoseidonServer]-> Received response from Ragnarok Online client [port2: " . $self->getPort . "] [Time: " . time . "]\n";
		$self->{state} = 'requested';
	
	} elsif ($switch eq '02A7') { # client sends hShield response
		# Queue the response
		$self->{response} = $msg;
		print "[PoseidonServer]-> Received response from Ragnarok Online client [port3: " . $self->getPort . "] [Time: " . time . "]\n";
		$self->{state} = 'requested';
